  ...
  try {
    ...
    if (!msg.fromMe && ticket.status === "pending") {
      ...
      if (ticket.queue && ticket.queue.storeAiId) {
        const gptQuery = await SendOpenAiQuery(ticket.queue.storeAiId, ticket.lastMessage)
      ...
      if (msg.type === "chat" && !chat.isGroup && !msg.hasMedia) {
        ...
        if (!handleTimeOut || finishTimeout < new Date() && ticket.queue && !ticket.queue.storeAiId) {
          ...
          if (lastSentMessage?.body != timeOutConfig!.notice.toString() && ticket.queue && !ticket.queue.storeAiId) {
